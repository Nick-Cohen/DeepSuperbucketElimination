...
BucketElimination::Bucket::Bucket(void)
	...
	_ChildVars(NULL), 
	_ApproxError_Local_Min(DBL_MAX), _ApproxError_Local_Avg(DBL_MAX), _ApproxError_Local_Max(DBL_MAX), 
	_ApproxError_Cmltv_Min(DBL_MAX), _ApproxError_Cmltv_Avg(DBL_MAX), _ApproxError_Cmltv_Max(DBL_MAX),
...
ARE::ARP *BucketElimination::Bucket::Problem(void) const 
	{ return nullptr == _Workspace ? nullptr : _Workspace->Problem() ; }
...
int32_t BucketElimination::Bucket::SaveXMLString(const char *prefixspaces, const std::string & Dir, std::string & S)
{
	...
	if (_OutputFunction.N() > 0 && NULL != _OutputFunctionBlockComputationResult) {
		...
		sprintf(s, "\n%s <ownbucketfncomputationresult nComputed=\"%lld/%lld\" bits=\"", prefixspaces, _nOutputFunctionBlocksComputed, nTB) ;
		S += s ;
		int32_t n = 0 ;
...
int32_t BucketElimination::Bucket::SetOriginalFunctions(int32_t N, ARE::Function *FNs[], bool InvalidateSignature) 
{
	if (NULL != _OriginalFunctions) {
		delete [] _OriginalFunctions ;
		_OriginalFunctions = NULL ;
	...
	if (InvalidateSignature) _OriginalWidth = -1 ;
	_nOriginalFunctions = 0 ;
	return AddOriginalFunctions(N, FNs, InvalidateSignature) ;
}


int32_t BucketElimination::Bucket::AddOriginalFunctions(int32_t N, ARE::Function *FNs[], bool InvalidateSignature) 
{
	if (N < 1) 
		return 0 ;
...
// 2021-11-10 : don't compute signature here; this computation seems wrong anyway; let the calling fn redo signature...
...
int32_t BucketElimination::Bucket::AddAugmentedFunctions(int32_t N, ARE::Function *FNs[], bool InvalidateSignature) 
{
	if (N < 1) 
		return 0 ;
	...
	for (i = N-1 ; i >= 0 ; i--) {
		ARE::Function *f = FNs[i] ;
		for (j = 0 ; j < _nAugmentedFunctions ; j++) {
			if (_AugmentedFunctions[j] == f) {
				FNs[i] = FNs[--N] ;
				break ;
				}
			}
	...
	// reallocate original functions array
	int32_t space = _nAugmentedFunctions + N ;
	ARE::Function **fnlist = new ARE::Function*[space] ;
	if (NULL == fnlist) 
		return 1 ;
	for (i = 0 ; i < _nAugmentedFunctions ; i++) 
		fnlist[i] = _AugmentedFunctions[i] ;
	for (; i < space ; i++) {
		fnlist[i] = FNs[i-_nAugmentedFunctions] ;
		fnlist[i]->SetBucket(this) ;
		}
	delete [] _AugmentedFunctions ;
	_AugmentedFunctions = fnlist ;
	_nAugmentedFunctions = space ;
...
int32_t BucketElimination::Bucket::ComputeSignature(unsigned char instructions)
{
	...
	if (_OriginalWidth < 0) {
		...
		else if (_Vars.size() > 0) {
			_OriginalSignature = new int32_t[_Vars.size()] ;
			if (NULL == _OriginalSignature) 
				return 1 ;
			for (int32_t i = 0 ; i < _Vars.size() ; i++) 
				_OriginalSignature[i] = _Vars[i] ;
			_OriginalWidth = _Vars.size() ;
			}
		if (_OriginalWidth < 0) 
			return 1 ;
		}
	...
	if (nF > 0) {
		...
		_Signature = new int32_t[_Vars.size()] ;
		if (NULL == _Signature) 
			return 1 ;
		for (int32_t i = 0 ; i < _Vars.size() ; i++) 
...
int32_t BucketElimination::Bucket::AddVarsToSignature(int32_t & n, int32_t *AdditionalSignature)
{
	int32_t i, j, k = 0 ;

	for (i = 0 ; i < n ; ++i) {
		int v = AdditionalSignature[i] ;
		for (j = 0 ; j < _Width ; ++j) {
			if (_Signature[j] == v) break ; }
		if (j >= _Width) 
			AdditionalSignature[k++] = v ;
		}
	n = k ;
	if (n <= 0) 
		return 0 ;

	int32_t w = (_Width < 0 ? 0 : _Width) + n ;
	int32_t *sig = new int32_t[w] ;
	if (nullptr == sig) 
		return 1 ;
	for (i = 0 ; i < _Width ; ++i) 
		sig[i] = _Signature[i] ;
	for (j = 0 ; j < n ; ++j) 
		sig[i++] = AdditionalSignature[j] ;
...
__int64 BucketElimination::Bucket::ComputeProcessingComplexity(bool ForMiniBuckets)
{
	__int64 N = 0 ;
	if (ForMiniBuckets) {
		for (MiniBucket *mb : _MiniBuckets) {
		...
		if (_Width < 0) {
			ComputeSignature(3) ; // include both original and augmented
			if (_Width < 0) 
				return -1 ;
			}
		N = 1 ;
...
__int64 BucketElimination::Bucket::ComputeEliminationComplexity(void)
{
	__int64 N = 1 ;
	for (int32_t i = _Vars.size()-1 ; i >= 0 ; --i) {
		N *= _Workspace->Problem()->K(_Vars[i]) ;
		}
	return N ;
}
...
int32_t BucketElimination::Bucket::ComputeOutputFunctions(bool DoMomentMatching, signed char approx_bound, int64_t & TotalSumOutputFunctionsNumEntries)
{
	...
	if (_MiniBuckets.size() > 1 && DoMomentMatching) {
		if (problem_is_optimization) {
			...
			int64_t table_size = max_marginals[0]->TableSize() ;
			if (table_size <= 0) 
				goto done_MM ;
...
done_MM :
	...
	if (0 != res) {
		int32_t res_error = Compute_ArroxError_Cmltv() ;
		}
...
int32_t BucketElimination::Bucket::CreateMBPartitioning(int32_t iBound, bool CreateTables, bool doMomentMatching, signed char Bound, bool noPartitioning, bool & AbandonIfMoreThan1MB, std::vector<int32_t> & key, std::vector<int64_t> & data, std::vector<int32_t> & helperArray)
{
	if (nullptr == _Workspace) 
		return 1 ;

	// iBound-1 is how many variables are allowed in the output fn scope...
	// _Vars.size() is how many are eliminated...
	int32_t maxNumVarsInMB ; maxNumVarsInMB = (iBound-1) + _Vars.size() ;
	if (noPartitioning) 
		maxNumVarsInMB = 1000000 ;

	bool abandonIfMoreThan1MB = AbandonIfMoreThan1MB ;
	AbandonIfMoreThan1MB = false ;

//int64_t tB = ARE::GetTimeInMilliseconds() ;

	// check that all child buckets have already been partitioned
	for (int32_t i = 0 ; i < _ChildVars.size() ; i++) {
		int32_t child = _ChildVars[i] ;
		Bucket *b = _Workspace->MapVar2Bucket(child) ;
		if (NULL == b) continue ;
		int32_t nOAfn = b->nOriginalFunctions() + b->nAugmentedFunctions() ;
		if (0 == nOAfn) 
			continue ; // no original/augmented functions -> no output fn
		if (b->nMiniBuckets() <= 0) 
			return 1 ;
		}
	...
	for (int32_t j = _nOriginalFunctions - 1 ; j >= 0 ; j--) {
		...
		int32_t nElimVars = f->ContainsVariables(_Vars) ;
		if (nElimVars <= 0) // must contain at least 1!
			goto failed ;
		int32_t n = f->N() - nElimVars ;
		key.push_back(-n) ;
		data.push_back((int64_t) f) ;
	...
	// do secondary sort so that functions with equal number of nonElimVars are also consistenty/persistently sorted
	int32_t j ; j = 0 ;
	for (int32_t i = 1 ; i <= data.size() ; ++i) {
		ARE::Function *f = i < data.size() ? (ARE::Function*) data[i] : nullptr ;
		int32_t f_key = i < key.size() ? key[i] : 1 ;
		if (nullptr != f ? f_key != key[j] : true) {
			int32_t n = i - j, k ;
			if (n > 1) {
...
/*
int64_t tS = ARE::GetTimeInMilliseconds() ;
if (_IDX < 2000) {
int64_t dt = tS - tB ;
...
//int64_t dtSUM1 = 0, dtSUM2 = 0, dtSUM3 = 0 ; ;
		...
		for (int32_t j = 0 ; j < key.size() ; ++j) {
			...
			if (iBound > 0) {
				for (MiniBucket *mb : _MiniBuckets) {
//int64_t t1 = ARE::GetTimeInMilliseconds() ;
					int32_t res = mb->AllowsFunction(*f, maxNumVarsInMB, iBound-1, helperArray) ;
//int64_t t2 = ARE::GetTimeInMilliseconds() ;
//dtSUM1 += t2 - t1 ;
					if (res < 0) 
						goto failed ;
					if (res > 0) {
						placed = true ;
//int64_t t1_ = ARE::GetTimeInMilliseconds() ;
						if (0 != mb->AddFunction(*f, helperArray)) 
							goto failed ;
//int64_t t2_ = ARE::GetTimeInMilliseconds() ;
//dtSUM2 += t2_ - t1_ ;
						break ;
						}
					}
...
//int ss = _MiniBuckets.size() ;
				if (_MiniBuckets.size() > 0 && abandonIfMoreThan1MB) {
					AbandonIfMoreThan1MB = true ;
					goto failed ;
					}
//int64_t t3 = ARE::GetTimeInMilliseconds() ;
				...
				mb->Vars() = _Vars ;
//				for (int32_t idxV = 0 ; idxV < _Vars.size() ; idxV++) 
//					mb->AddVar(_Vars[idxV]) ;
				if (0 != mb->AddFunction(*f, helperArray)) 
					goto failed ;
//int64_t t4 = ARE::GetTimeInMilliseconds() ;
//dtSUM3 += t4 - t3 ;
				}
			}

/*int64_t tMB = ARE::GetTimeInMilliseconds() ;
...
/*int64_t tE = ARE::GetTimeInMilliseconds() ;
if (_IDX < 2000) {
int64_t dt = tE - tMB ;
...
::fflush(ARE::fpLOG) ; }*/
	...
	return 0 ;
failed :
	fprintf(ARE::fpLOG, "\n   BucketElimination::Bucket::CreateMBPartitioning ERROR : failed label reached (AbandonIfMoreThan1MB=%c) ...", AbandonIfMoreThan1MB ? 'Y' : 'N') ;
	::fflush(ARE::fpLOG) ;
	DestroyPartitioning() ;
	return 1 ;
}
...
int32_t BucketElimination::Bucket::ComputeFirstVariableDistributionEx(int32_t *ContextValues, ARE_Function_TableType *dist)
{
	...
	for (j = 0 ; j < nIF ; j++) {
		...
		if (0 == f->N()) bews->ApplyFnCombinationOperator(const_factor, f->ConstValue()) ;
			...
			int64_t adr = f->ComputeFnTableAdr(ContextValues, problem->K()) ;
	...
	for (int32_t i = 0 ; i < problem->K(_V) ; i++) {
		...
		for (j = flist.size() - 1 ; j >= 0 ; j--) { // note : it should be that 0 != flist[j]->N(). note : it is assumed that flist[j] has 1 block. note : it is assumed that order of flist[j] arguments is the same as signature.
			int64_t adr = flist[j]->ComputeFnTableAdr(ContextValues, problem->K()) ;
