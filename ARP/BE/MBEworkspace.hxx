...
protected :
	...
	Bucket **_Var2BucketMapping ; // bucket for each variable; this is the bucket where the variable is eliminated...
...
//	int32_t *_BTchildlistStorage = NULL ;
	...
	/*
		EClimit is the max complexity of eliminting the variables of this bucket; default = -1 (NA)...
	*/
	int64_t _EClimit ;

	/*
		i-bound limits the number of (mini-)bucket output function arguments
		number of output function arguments = iBound-1
	*/
	int32_t _iBound ; // iBound-1 is the max num of (mini-)bucket output fn arguments; default is 1000000 = meaning infinite
...
public :
	...
	inline int64_t & EClimit(void) { return _EClimit ; }

	inline int32_t VarOrdering_MaxCliqueSize(void) const { return _VarOrdering_MaxCliqueSize ; }
	...
	inline void SetVarOrdering_MaxCliqueSize(int32_t w) {  _VarOrdering_MaxCliqueSize = w ; }
...
protected :
	...
	int32_t _nBucketsWithSingleChild_initial ; // number of buckets with a single child; initially after problem is loaded (before superbuckets)...
	int32_t _nBucketsWithSingleChild_final ; // number of buckets with a single child; after superbuckets...
	int32_t _nBuckets_initial ;
	int32_t _nBuckets_final ;
	int32_t _nVarsWithoutBucket ; // variables that are removed from the problem; i.e. never processed during BE...
	int32_t _nConstValueFunctions ;

	int32_t _nRoots ; // number of roots in the bucket tree
	std::vector<int32_t> _RootVars ;
	...
	int32_t _MaxNumVarsInBucket ; // maximum number of variables in any bucket; this includes vars to be eliminated...
	int32_t _MaxNumVarsElimInBucket ; // maximum number of variables eliminated in a bucket...
	int32_t _nLeafBucketsWithNoFunctions ; // number of leaf buckets with no functions; these would normally get thrown away...
	int64_t _MaxElimComplexityInBucket ; // maximum number of elim combinations per bucket...
	...
	int32_t _nSBmerges ; // number of SuperBuckets merges...
...
public :
	...
	inline int32_t MaxNumVarsElimInBucket(void) const { return _MaxNumVarsElimInBucket ; }
	inline int64_t MaxElimComplexityInBucket(void) const { return _MaxElimComplexityInBucket ; }
	...
	inline int32_t nSBmerges(void) const { return _nSBmerges ; }
	...
	// this fn can be run any time, whether MB partitioning is done or not
	BucketElimination::Bucket *GetBucketWithMostElimVariables(void) ;
	...
	int32_t ComputeNumRoots(void)
	{
		...
		_RootVars.clear() ;
		if (_RootVars.capacity() < 100) {
			_RootVars.reserve(100) ;
			if (_RootVars.capacity() < 100) 
				return 1 ;
			}
...
//				continue ; // don't count evidence variables
			if (NULL == b->ParentBucket()) {
				for (int32_t j = b->nVars()-1 ; j >= 0 ; --j) 
					_RootVars.push_back(b->Var(j)) ;
				_nRoots++ ;
				}
			}
		return 0 ;
	}
	inline void DestroyAugmentedFunctions(void)
	{
		for (int32_t i = 0 ; i < _nBuckets ; i++) {
			BucketElimination::Bucket *b = _Buckets[i] ;
			if (NULL == b) continue ;
	...
	inline int32_t TidyBucketsArray(void)
	// move buckets ptrs so that there are no NULL ptrs in the array...
	// sort elimination variables...
	{
		int32_t i, j, k ;
		for (j = i = 0 ; i < _nBuckets ; ++i) {
			...
			_Buckets[j] = b ;
			_Buckets[j]->SetIDX(j) ;
			_Buckets[j]->SortVars() ; // sort elimination variables
			++j ;
			}
		_nBuckets = j ;
		return 0 ;
...
public :
	...
	virtual int32_t CreateBuckets(bool ANDORtree, bool KeepBTsignature, bool SimplifyBTstructure) ;

	// if _EClimit is > 0, then process buckets and merge some ...
	virtual int32_t CreateSuperBuckets(void) ;
	...
	virtual int32_t ComputeBucketTreeStatistics(void)
	{
		...
		_MaxElimComplexityInBucket = 0 ;
		_MaxNumVarsElimInBucket = 0 ;
		...
		for (i = 0 ; i < _nVars ; ++i) {
			if (nullptr == _Var2BucketMapping[i]) 
				_nVarsWithoutBucket++ ;
			}
		...
		for (i = 0 ; i < _nBuckets; i++) {
			BucketElimination::Bucket *b = _Buckets[i] ;
			if (nullptr == b) 
				continue ; // could be that SuperBuckets deleted this bucket...
		...
		int32_t nRoots = nAdded ; // check _nRoots == nAdded
		...
		for (BucketElimination::Bucket *b = BLhead ; NULL != b ; b = b->NextInOrder()) {
			_nOriginalFunctions += b->nOriginalFunctions() ;
			_nAugmentedFunctions += b->nAugmentedFunctions() ;
			if (b->Width() < 0) 
				b->ComputeSignature() ;
			if (b->Width() > _MaxNumVarsInBucket) 
				_MaxNumVarsInBucket = b->Width() ;
			int64_t elimC = b->ComputeEliminationComplexity() ;
			if (elimC > _MaxElimComplexityInBucket) 
				_MaxElimComplexityInBucket = elimC ;
			if (b->nVars() > _MaxNumVarsElimInBucket) 
				_MaxNumVarsElimInBucket = b->nVars() ;
			B = b->ParentBucket() ;
			if (b->Height() < 0) 
				b->SetHeight(0) ;
			if (b->nChildren() > 1) 
	...
	int32_t ComputeOutputFunctions(bool DoMomentMatching, bool LogConsole, signed char ApproxBound /* upper/lower? */, int64_t & TotalSumOutputFunctionsNumEntries) ;
	...
	// create MB partitionin.
	// in each MB, the number of non-elimination variables is iBound-1
	int32_t CreateMBPartitioning(bool CreateTables, bool doMomentMatching, signed char Bound /* e.g. upper/lower/approx */, bool noPartitioning, int32_t ComputeComputationOrder) ;
	// destroy MB partitionin.
