...
int32_t ARE::ARP::LoadFromBuffer(const char *format, const char *buf, int32_t L)
{
	...
	if (NULL != _fpLOG) {
		...
		fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); L=%d ...", tNOW, L) ;
		fflush(_fpLOG) ;
		}
	...
	if (0 == stricmp("UAI", format)) {
		if (NULL != _fpLOG) {
			int64_t tNOW = ARE::GetTimeInMilliseconds() ;
			fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); format is UAI ...", tNOW) ;
			fflush(_fpLOG) ;
			}
		...
		if (5 == i ? 0 == memcmp(buf, "BAYES", i) : false) {
			if (NULL != _fpLOG) {
				int64_t tNOW = ARE::GetTimeInMilliseconds() ;
				fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); UAI type is BAYES ...", tNOW) ;
				fflush(_fpLOG) ;
				}
			...
			if (NULL != _fpLOG) {
				int64_t tNOW = ARE::GetTimeInMilliseconds() ;
				fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); UAI type is MARKOV ...", tNOW) ;
				fflush(_fpLOG) ;
				}
			...
			if (NULL != _fpLOG) {
				int64_t tNOW = ARE::GetTimeInMilliseconds() ;
				fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); UAI type is SPARSE ...", tNOW) ;
				fflush(_fpLOG) ;
				}
			i = LoadUAIFormat(buf+7, L-7) ;
			}
			...
			if (NULL != _fpLOG) {
				int64_t tNOW = ARE::GetTimeInMilliseconds() ;
				fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); UAI type is unknown ...", tNOW) ;
				fflush(_fpLOG) ;
				}
		...
		if (NULL != _fpLOG) {
			int64_t tNOW = ARE::GetTimeInMilliseconds() ;
			fprintf(_fpLOG, "\n%lld ARE::ARP::LoadFromBuffer(); format is uknown ...", tNOW) ;
			fflush(_fpLOG) ;
			}
		return ERRORCODE_problem_type_unknown ;
		}
