...
/*		char s[1024] ;
		...
		fprintf(ARE::fpLOG, "\nBEEM Task Generator : stats : \n   hasErrors=%c \n   nTableBlocksLoaded=%lld nTableBlocksSaved=%lld \n   CurrentDiskMemorySpaceCached=%lld MaximumDiskMemorySpaceCached=%lld \n   CurrentNumDiskBlocksInMemory=%d maxNumConcurDiskBlocksInMemory=%d", 
			(char) (hasErrors ? 'Y' : 'N'), 
		...
		fprintf(ARE::fpLOG, "\n   InputTableBlockWaiting=(%lld times, %lld millisec) ", nBlocksWaited, tw) ;
		...
		if (NULL != bews) {
			...
			for (i = 0 ; i < bews->nBuckets() ; i++) {
				...
				fprintf(ARE::fpLOG, "\n bucket %d OUT : n blocks = %lld, n times loaded %d", (int) i, nblocks, _nFTBsLoadedPerBucket[i]) ;
				}
			}*/
		fflush(ARE::fpLOG) ;
		}
