...
static int ReadDirectory(const char *Dir, const char *Pattern, const char *prefix, const char *ext, std::vector<std::string> & Filenames)
{
    ...
    while ((pEnt = readdir(pDir)) != NULL) {
        ...
        Filenames.push_back(fName); // stringToWstring(fName)) ;
		}
    closedir(pDir) ;
#endif
...
is_better :
	...
	if (NULL != _fpLOG) {
		fprintf(_fpLOG, "\n%lld worker %2d found better solution : width=%d complexity=%g bt_height=%d space(#elements)=%g", tNow, (int) w_IDX, (int) G._VarElimOrderWidth, (double) G._TotalVarElimComplexity_Log10, (int) bt_height, (double) G._TotalNewFunctionStorageAsNumOfElements_Log10) ;
		fflush(_fpLOG) ;
		}
#ifdef VERBOSE_CVO
	printf("%lld worker %2d found better solution : width=%d complexity=%g BTh=%d BTib=%d nBTp=%d maxBTPd=%d space(#elements)=%g\n", tNow, (int) w_IDX, (int) G._VarElimOrderWidth, (double) G._TotalVarElimComplexity_Log10, (int) bt_height, (int) ib, (int) nBTpartitionings, (int) bt_maxDepthPartitionedNode, (double) G._TotalNewFunctionStorageAsNumOfElements_Log10) ;
#endif
...
#endif 
{
	...
	if (NULL != CVOcontext._fpLOG) {
		...
		fprintf(CVOcontext._fpLOG, "\n%lld worker %2d start; objCode1=%d objCode2=%d alg=%d ...", tNow, (int) w->_IDX, (int) CVOcontext._ObjCodePrimary, (int) CVOcontext._ObjCodeSecondary, (int) CVOcontext._AlgCode) ;
	...
	try {
		*(w->_G) = CVOcontext._MasterGraph ;
		...
		if (NULL != CVOcontext._fpLOG) {
			tNow = ARE::GetTimeInMilliseconds() ;
			fprintf(CVOcontext._fpLOG, "\n%lld worker %2d prep0 exception ...", tNow, (int) w->_IDX) ;
			fflush(CVOcontext._fpLOG) ;
			}
		}
	...
	while (! w->_ThreadStop) {
		...
		if (0 == (v % CVOcontext._LogIncrement)) {
			if (NULL != CVOcontext._fpLOG) {
				tNow = ARE::GetTimeInMilliseconds() ;
				fprintf(CVOcontext._fpLOG, "\n%lld worker %2d will do run %d ...", tNow, (int) w->_IDX, (int) v) ;
				fflush(CVOcontext._fpLOG) ;
				}
			}
// DEBUGGG
...
//ARE::VarElimOrderComp::DeleteNewAdjVarList(w->_TempAdjVarSpaceSizeExtraArrayN, w->_TempAdjVarSpaceSizeExtraArray) ;
			...
			if (NULL != CVOcontext._fpLOG) {
				tNow = ARE::GetTimeInMilliseconds() ;
				fprintf(CVOcontext._fpLOG, "\n%lld worker %2d exception ...", tNow, (int) w->_IDX) ;
				fflush(CVOcontext._fpLOG) ;
				goto done ;
				}
			}
// DEBUGGG
//		printf("\nworker %d finished res=%d; width=%d complexity=%lld", (int) w->_IDX, (int) res, (int) w->_G->_VarElimOrderWidth, (int64_t) w->_G->_TotalVarElimComplexity) ;
//		if (NULL != CVOcontext._fpLOG) {
//			fprintf(CVOcontext._fpLOG, "\nworker %d finished res=%d; width=%d complexity=%lld", (int) w->_IDX, (int) res, (int) w->_G->_VarElimOrderWidth, (int64_t) w->_G->_TotalVarElimComplexity) ;
//			fflush(CVOcontext._fpLOG) ;
//			}
...
//printf("\n%s worker %2d found width=%d complexity=%lld space(#elements)=%lld res=%d", strDT, (int) w->_IDX, (int) w->_G->_VarElimOrderWidth, (int64_t) w->_G->_TotalVarElimComplexity, (int64_t) w->_G->_TotalNewFunctionStorageAsNumOfElements, (int) res) ;
			if (0 == res) {
...
//					printf("\nworker %d out of time ...", (int) w->_IDX) ;
			...
			if (NULL != CVOcontext._fpLOG) {
				tNow = ARE::GetTimeInMilliseconds() ;
				fprintf(CVOcontext._fpLOG, "\n%lld worker %d summary exception ...", tNow, (int) w->_IDX) ;
				fflush(CVOcontext._fpLOG) ;
				}
			}
		...
		try {
			*(w->_G) = CVOcontext._MasterGraph ;
			...
			if (NULL != CVOcontext._fpLOG) {
				tNow = ARE::GetTimeInMilliseconds() ;
				fprintf(CVOcontext._fpLOG, "\n%lld worker %d prep exception ...", tNow, (int) w->_IDX) ;
				fflush(CVOcontext._fpLOG) ;
				}
			}
		}
	...
	if (NULL != CVOcontext._fpLOG) {
		tNow = ARE::GetTimeInMilliseconds() ;
		fprintf(CVOcontext._fpLOG, "\n%lld worker %d done ...", tNow, (int) w->_IDX) ;
		fflush(CVOcontext._fpLOG) ;
		}
	w->_ThreadHandle = 0 ;
...
#endif 
{
	...
	if (NULL == BestOrder._VarListInElimOrder) {
		if (0 != BestOrder.Initialize(p.N())) {
			ret = 1000 ;
			if (NULL != context->_fpLOG) {
				fprintf(context->_fpLOG, "\n%lld CVO control thread; bestorder init failed ...", tNow) ;
				fflush(context->_fpLOG) ;
				}
			goto done ;
			}
	...
	tNow = ARE::GetTimeInMilliseconds() ;
	if (NULL != context->_fpLOG) {
		fprintf(context->_fpLOG, "\n%lld CVO control thread; eliminate easy vars from graph ...", tNow) ;
		fflush(context->_fpLOG) ;
		}
...
//ARE::VarElimOrderComp::DeleteNewAdjVarList(context->_TempAdjVarSpaceSizeExtraArrayN, context->_TempAdjVarSpaceSizeExtraArray) ;
	...
	tNow = ARE::GetTimeInMilliseconds() ;
	if (NULL != context->_fpLOG) {
		fprintf(context->_fpLOG, "\n%lld CVO control thread; %d vars eliminated, %d remaining ...", tNow, (int) MasterGraph._OrderLength, (int) MasterGraph._nRemainingNodes) ;
		fflush(context->_fpLOG) ;
		}
	...
	if (NULL != context->_fpLOG) {
		fprintf(context->_fpLOG, "\n%lld CVO control thread; preprocessing done; ready to run : nRunsToDo=%d/%d, tLimitmsec=%lld (tAutoStop=%lld) ...", tNow, context->_nRunsToDoMin, context->_nRunsToDoMax, context->_TimeLimitInMilliSeconds, context->_tToStop) ;
		fflush(context->_fpLOG) ;
		}
...
//	if (! context->_FindPracticalVariableOrder) {
	...
	if (0 != stop_signalled) {
		if (NULL != context->_fpLOG) {
			tNow = ARE::GetTimeInMilliseconds() ;
			fprintf(context->_fpLOG, "\n%lld CVO control thread; ready to create workers, but stop signalled ...", tNow) ;
			fflush(context->_fpLOG) ;
			}
		goto done ;
		}
	...
	if (NULL != context->_fpLOG) {
		tNow = ARE::GetTimeInMilliseconds() ;
		fprintf(context->_fpLOG, "\n%lld CVO control thread; nWorkers=%d", tNow, nWorkers) ;
		fflush(context->_fpLOG) ;
		}
...
//goto done ;
	...
	if (NULL == Workers) {
		...
		if (NULL != context->_fpLOG) {
			tNow = ARE::GetTimeInMilliseconds() ;
			fprintf(context->_fpLOG, "\n%lld CVO control thread; failed to create workers, will quit ...", tNow) ;
			fflush(context->_fpLOG) ;
			}
		goto done ;
		}
	...
	for (i = 0 ; i < nWorkers ; i++) {
		...
		if (0 == Workers[i]._ThreadHandle) {
			...
			if (NULL != context->_fpLOG) {
				tNow = ARE::GetTimeInMilliseconds() ;
				fprintf(context->_fpLOG, "\n%lld CVO control thread; FAILED to create thread %d", tNow, i) ;
				fflush(context->_fpLOG) ;
				}
			}
		else 
	...
	if (0 == nWrunning) {
		if (NULL != context->_fpLOG) {
			tNow = ARE::GetTimeInMilliseconds() ;
			fprintf(context->_fpLOG, "\n%lld CVO control thread; no worker threads, will quite ...", tNow) ;
			fflush(context->_fpLOG) ;
			}
		ret = 1006 ;
		goto done ;
	...
	while (true) {
		...
		if (0 != stop_signalled) {
			if (NULL != context->_fpLOG) {
				tNow = ARE::GetTimeInMilliseconds() ;
				fprintf(context->_fpLOG, "\n%lld CVO control thread; stop_signalled ...", tNow) ;
				fflush(context->_fpLOG) ;
				}
			}
		nRunning = 0 ;
		for (i = 0 ; i < nWorkers ; i++) {
			...
			if (0 != stop_signalled) {
				if (NULL != context->_fpLOG) {
					tNow = ARE::GetTimeInMilliseconds() ;
					fprintf(context->_fpLOG, "\n%lld CVO control thread; stopping worker %d ...", tNow, i) ;
					fflush(context->_fpLOG) ;
					}
				Workers[i]._ThreadStop = true ;
				}
	...
	if (NULL != context->_fpLOG) {
		tNow = ARE::GetTimeInMilliseconds() ;
		fprintf(context->_fpLOG, "\n%lld CVO control thread; all worker threads have closed ...", tNow) ;
		fflush(context->_fpLOG) ;
		}
...
*/
	...
	if (NULL != context->_fpLOG) {
		fprintf(context->_fpLOG, "\n%lld CVO control thread; done; time=%lldmsec ...", tNow, (int64_t) (context->_tEnd - context->_tStart)) ;
		fflush(context->_fpLOG) ;
		}
//	for (i = 0 ; i < nWorkers ; i++) 
...
int ARE::VarElimOrderComp::CVOcontext::RequestStopCVOthread(void)
{
	if (0 == _ThreadHandle) {
		if (NULL != _fpLOG) {
			...
			fprintf(_fpLOG, "\n%lld CVO_th : request stop variable order computation; already stopped ...", tNowLog) ;
			fflush(_fpLOG) ;
			}
		return 0 ;
		}
...
int ARE::VarElimOrderComp::CVOcontext::StopCVOthread(int64_t TimeoutInMilliseconds)
{
	if (0 == _ThreadHandle) {
		if (NULL != _fpLOG) {
			int64_t tNowLog = ARE::GetTimeInMilliseconds() ;
			fprintf(_fpLOG, "\n%lld CVO_th : stop variable order computation; already stopped ...", tNowLog) ;
			fflush(_fpLOG) ;
			}
		return 0 ;
		}
	...
	if (NULL != _fpLOG) {
		fprintf(_fpLOG, "\n%lld    CVO_th : stop variable order computation; stop signalled, will wait ...", tStart) ;
		fflush(_fpLOG) ;
		}
	...
	while (true) {
		...
		if (dt > TimeoutInMilliseconds) {
			...
			if (NULL != _fpLOG) {
				int64_t tNowLog = ARE::GetTimeInMilliseconds() ;
				fprintf(_fpLOG, "\n%lld CVO_th : stop variable order computation, hard kill ...", tNowLog) ;
				fflush(_fpLOG) ;
				}
#if defined WINDOWS || _WINDOWS
...
int32_t ARE::VarElimOrderComp::ComputeTreeDecomposition(ARE::ARP & P, int32_t *Order, int32_t Width, BucketElimination::MBEworkspace & bews)
{
	...
	if (0 != res_BEwsInit) 
		return 1 ;
	int res_BEwsCB = bews.CreateBuckets(true, true, true) ;
	if (0 != res_BEwsCB) 
		return 1 ;
...
//		char fn[260] ;
	...
	for (int32_t ii = 0 ; ii < files2process.size() ; ++ii) {
		...
		int res = ARE::VarElimOrderComp::Compute(fn,
			...
			PerformSingletonConsistencyChecking, EliminateSingletonDomainVariables, earlyterminationofbasic_W, earlyterminationofbasic_C, findPracticalVariableOrder, randomGeneratorSeed, vQuery, 
			BestOrder, context) ;
